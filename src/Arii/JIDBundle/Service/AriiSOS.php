...
class AriiSOS
{
    ...
    private function GetConnection($spooler) {
        if ($spooler=='')
            throw new \Exception('JID',10);
        
        // Recuperer simplement dans la table des connexions
        $Node = $this->portal->getNodeByName($spooler);
        if (!$Node)
            throw new \Exception('JID',10);

        # On recherche une porte d'entrée
        if (!$Node['Connections'])
            throw new \Exception('JID',11);
        
        $host = '';
        foreach ($Node['Connections'] as $Connection) {
            if ($Connection['protocol']=='ojs') {
                $protocol = 'http';
                $host = ($Connection['interface']!=''?$Connection['interface']:$Connection['host']);
                $port = ($Connection['port']!=''?$Connection['port']:4444);                
            }
        }        
        if ($host=='')
            throw new \Exception('JID',11);
    ...
    {
        $url = $this->GetConnection($spooler);
        ...
        if ($method=='CURL') {
            ...
            if (!$fp)
            {
                $this->portal->AuditLog($host, $cmd, "ERROR","JID", "!ERROR: Can not Open URL: ".$url);
        ...
        $this->portal->AuditLog($spooler, $cmd, "SUCCESS","JID", $msg);
    ...
    public function Log($spooler,$cmd)
    {
        $url = $this->GetConnection($spooler);
...
<body><pre>';
        $flag=0;
        foreach (explode("\n",$content) as $line) {
            if (substr($line,0,6)=='<span ') $flag=1; 
            if ($flag==1) {
                $log .= "$line\n";
            }
        }
