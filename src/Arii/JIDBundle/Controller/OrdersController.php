...
//                                $cell .= '<cell><![CDATA['.$job_name.' '.$state_text.']]></cell>';
    ...
    public function atomAction($history_max=0,$nested=false,$only_warning=true,$sort='last')
    {
        $request = Request::createFromGlobals();
        if ($request->get('history')>0) {
            $history_max = $request->get('history');
        ...
        $response = new Response();
        $response->headers->set('Content-Type', 'text/xml');
        return $this->render('AriiJIDBundle:RSS:orders.atom.twig', array( 'Infos' => $Orders), $response );
    }
