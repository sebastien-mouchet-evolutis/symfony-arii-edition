...
my ($config, $hotfolder, $order, $job_chain, $job, $config_file, $events, $show_params, $show_events, $show_def, $show_doc, $show_time, $show_chains, $show_jobs, $show_config, $status, $scheduler_xml, $images, $path, $file, $event_file, $debug, $lang, $code, $target, $splines, $rankdir, $schedule,  $help) = 
   (Basename($ENV{SCHEDULER_INI}), 'live', '^$', '^$', '^$', '^$', 'y', 'y', 'n', 'n', 'n', 'n', 'n', 'n', 'n', '', '', 'images', '.*',  '.',  '.*', '', 'fr', 'n', '', 'polyline', 'TB', ''  );
...
my(%Done);   # Tout ce qui a déjà été fait
...
if (opendir(DIR, $objdir.'/'.$path)) {
	...
	foreach my $f (@FILES) {
		if ($f =~ /$order\.order\.xml$/) {
			...
			if ($f =~ /^(.*),/) {
				$job_chain = $1;
				ReadChain($path.'/'.$job_chain.'.job_chain.xml',1);				}
		}
	}
...
my($f,$alone) = @_;
	...
	# Pour les ordres et le show config
        my $first_node = '';
        if ($show_config ne 'n') {
            $alone = 1;
            $first_node = CleanFilename2("$f","HEAD");
        }
	...
	# Tete de la chaine
	my($chain_node) = CleanFilename2("$f","HEAD");
        # on conserve les chaines liées aux etats  
        my %SubChain;
	...
	$label .= '</TABLE>';
	print '"'.$chain_node.'" [label=<'.$label.'>,URL="javascript:parent.DisplayFile(\''.$f.'.job_chain.xml\');"]'."\n"
            if ($alone);
	...
	# Liste des noeuds
        # On garde les etats des noeuds de fin
        my %Success;
        my %Error;
        my @EndNodes;
	if ($doc->{job_chain}->{job_chain_node}) {
		...
		foreach $node ( @NODES ) {
			...
			PrintNode( $chain_node );
                        ...
                        if ($first_node eq '') {
                            ...
                            # si c'est une boite seule, on la relie a la tete
                            if ($alone) {
                                PrintLink(CleanFilename("$f/HEAD"),$first_node,'[color=grey,style=dashed]');
                            }
                        }
			...
			# Nouvelle norme SOS-Berlin
                        my $parent = $node->{state}; 
                        if ($parent =~ s/(.*?)://) {
                            my $father = $1;
                            PrintLink ( CleanFilename("$f/$father"), $chain_node, '[color=yellow,style=dashed]' );
                        }
			...
			if ($node->{job}) {
				...
				# On ajoute le job
                                if ($show_jobs ne 'n') {
                                    ...
                                    ReadJob($joblink.'.job.xml',1);
                                }
                                else {
                                    $label .= ReadJob($joblink.'.job.xml',0);
                                }
			}                        

			$label .= '</TABLE>';
                        # 
			PrintNode ($chain_node, '[label=<'.$label.'>,URL="javascript:parent.DisplayFile(\''.$file.'\');"]');
		}
	}
		...
		foreach $node ( @NODES ) {
			#my($chain_node) = CleanFilename("$f/".$node->{state});
                        my($chain_node) = CleanFilename2("$f",$node->{state});
			PrintNode ( $chain_node );
			...
			if ($node->{job_chain}) {
				...
				# On stocke les liens pour ?viter qu'il se retrouvent dessin?s dans la chaine
                                ...
                                if ($show_chains ne 'n') {
                                    if ($show_chains eq 'n') {                                    
                                        my ($fn, %Suc, %Err, @End ) = ReadChain(  CleanFilename2(dirname("$f"),$node->{job_chain}).'.job_chain.xml',0);
                                        PrintLink( $chain_node, $fn, "[style=dashed]", $unicID );

                                    }
                                    else {
                                        PrintLink( $chain_node, CleanFilename2(dirname("$f"),$node->{job_chain}).'/HEAD', "[style=dashed]", $unicID );
                                        push( @CHAINS,  CleanFilename2(dirname("$f"),$node->{job_chain}));
                                    }
                                }
			}
			...
			# Prochaines etapes
                        ...
                        if ($node->{next_state}) {
                                $next = $node->{next_state};
                                PrintLink( $chain_node, CleanFilename2("$f",$next), "[color=green,style=dashed]" );
                                $Success{$next}++;
                        }
                        if ($node->{error_state}) {
                                $next = $node->{error_state};
                                PrintLink( $chain_node, CleanFilename2("$f",$next), "[color=red,style=dashed]" );
                                $Error{$next}++;
                        }
	...
	if ($doc->{job_chain}->{file_order_sink}) {
        ...
        # Avant de fermer la chaine, on complete par la config
        if ($show_config ne 'n') {
...
return ($first_node,%Success,%Error,@EndNodes, $chain_node);
}
# ---------------------------------------------
# ReadJob()
# ---------------------------------------------
...
my($f,$alone) = @_;
	...
	if ($doc->{job}->{order}) {
        ...
        my $label;
        $label = '<TABLE BORDER="1" CELLBORDER="0" CELLSPACING="0" COLOR="grey" BGCOLOR="'.$BGColor{$jobtype}.'">'."\n"
            if ($alone);
