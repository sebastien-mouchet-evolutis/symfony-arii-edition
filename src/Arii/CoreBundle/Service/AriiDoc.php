...
class AriiDoc
{
    ...
    public function __construct (AriiPortal $portal, RequestStack $requestStack ) {
        $Parameters = 

        $this->java     = $portal->getParameter('java');
        $this->ditaa    = $portal->getParameter('ditaa');
        $this->graphviz = $portal->getParameter('graphviz_dot');
    ...
    public function Ditaa($text,$name,$filename='') {
        ...
        if ($update) {            
            ...
            $cmd = '"'.$this->java.'/bin/java" -Dfile.encoding=UTF-8 -jar ../java/'.$this->ditaa." \"$input.ditaa\" \"$output\"";
