...
class AriiSQL
{
    ...
    public function setDriver($driver) {
        $this->driver = $driver;
        $this->Init($this->portal,$driver);
        return $driver;
    }
       
    // Force la base de données
    public function InitDB($db) {
        $this->Init($this->portal, $db);
    }
    
    protected function Init($portal,$driver) {
        ...
        switch ($this->driver) {
            ...
            case 'oracle':
            case 'pdo_oci':    
                $this->past = "to_date('".$User['past']."','yyyy-mm-dd hh24:mi:ss')";
                $this->future = "to_date('".$User['future']."','yyyy-mm-dd hh24:mi:ss')";
                break;
            default:
                $this->past   = '"'.$User['past'].'"';
                $this->future = '"'.$User['future'].'"';
   ...
   public function LeftJoin( $table, $Columns, $operator='=' ) {
       $Cols = array();
       foreach ($Columns as $c ) {
       ...
       if ($operator == 'like')
           $operator = ' like ';
       return ' left join '.$table.' on '.implode($operator,$Cols);
   }
           ...
           foreach (array_keys($Right) as $k ) {
                if ($k == 'job|job_chain') {
                   ...
                   array_push($Cols,$pass.'('.implode(' or ',$Special).')');
                    ...
                    if (isset($line[$k]) and ($line[$k]!='') and ($line[$k]!='%')) {
                        $val = $line[$k];
                        if (strpos(" $val",'%')>0) $operator='like';
                            else $operator='=';
