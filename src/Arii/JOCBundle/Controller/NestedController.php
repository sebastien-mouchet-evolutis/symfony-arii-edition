...
/*******************************************/
    ...
    public function list_xmlAction()
    {
        ...
        $qry = $sql->Select(array( 
                ...
                .$sql->LeftJoin('JOC_JOB_CHAIN_NODES fjcn',array('fjcn.job_chain_id','fjc.id'))
                .$sql->LeftJoin('JOC_JOB_CHAINS fjc2',array('fjcn.chain_id','fjc2.id'))
                .$sql->LeftJoin('JOC_JOB_CHAIN_NODES fjcn2',array('fjcn2.job_chain_id','fjc2.id'))
                .$sql->LeftJoin('JOC_SPOOLERS fs',array('fjc.spooler_id','fs.id'))  
                .$sql->LeftJoin('JOC_JOBS fj',array('fjcn2.job_id','fj.id'))
                .$sql->LeftJoin('JOC_ORDERS fo',array('fjcn2.id','fo.job_chain_node_id'))
                .$sql->LeftJoin('JOC_FILE_ORDERS ff',array('fjc2.id','ff.job_chain_id'))
                .$sql->LeftJoin('JOC_TASKS ft',array('fo.task_id','ft.id'))
                .$sql->Where( $Fields )
