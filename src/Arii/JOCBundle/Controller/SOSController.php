        ...
        switch ($xml_command) {    
            ...
            case 'start_job':
                ...
                if ($request->get('id')!='') {
                    ...
                    list($spooler_id,$protocol,$hostname,$port,$path,$job) = $sos->getJobInfos($job_id);
                }
                else {
                    return "?! job";
                }
...
// http://www.sos-berlin.com/mediawiki/index.php/What_is_the_difference_between_%22end%22_and_%22kill_immediately%22%3F
   ...
   public function syncAction($what='orders,job_chain_orders,job_orders,jobs,job_chains,schedules,remote_schedulers,payload,job_params') {
        ...
        $focus->cache($data,'WEB',-1);
        print $this->get('translator')->trans('Synchronized');
        exit();
    }

   public function syncAllAction($what='orders,job_chain_orders,job_orders,jobs,job_chains,schedules,remote_schedulers,payload,job_params') {
        set_time_limit ( 30 );
        ...
        $db = $dhtmlx->Connector('data');
        $qry = 'select NAME,IP_ADDRESS,TCP_PORT from JOC_SPOOLERS order by UPDATED';
        $res = $db->sql->query($qry);
        $status = '';
        $protocol = 'http';
        $path = '';
        
        // on traite en moins de 30s ?
        $n=0;
        while ($line = $db->sql->get_next($res) and ($n<1))
        {
            ...
            $data = $SOS->XMLCommand($scheduler,$hostname,$port,$path,$protocol,$cmd,'xml');
            if (isset($data['ERROR'])) {
                $status .= "<font color='red'>$scheduler !</font></br>";
            }
            else {
                $focus = $this->container->get('arii_joc.focus2');        
                $cache = $focus->cache($data,'WEB',-1);
