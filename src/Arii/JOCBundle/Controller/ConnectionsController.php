...
class ConnectionsController extends Controller {
    ...
    public function gridAction()
    {
        ...
        $qry = $sql->Select(array('fs.NAME as SPOOLER','fp.ID','fp.OPERATION_TYPE','fp.RESPONSES','fp.RECEIVED_BYTES','fp.SENT_BYTES','fp.STATE','fp.HOST_IP','fp.PORT'))
            .$sql->From(array('JOC_CONNECTIONS fp'))
            .$sql->LeftJoin('JOC_SPOOLERS fs', array('fp.SPOOLER_ID','fs.ID'))
        ...
        while ($line = $data->sql->get_next($res))
        {
            ...
            $list .= '<cell>'.$line['SENT_BYTES'].'</cell>';
            $list .= '</row>';
        }
        $list .= "</rows>\n";
