 ...
 *********************************************************************/
   ...
   public function getOrderInfos($id) {   
        ...
        $qry = $sql->Select(array(  'fo.PATH','fo.NAME as ORDER_ID','fo.STATE','fo.INITIAL_STATE','fo.END_STATE',
                                    'fs.NAME as SPOOLER_ID'))
                .$sql->From(array('JOC_ORDERS fo'))
                .$sql->LeftJoin('JOC_JOB_CHAIN_NODES fcn', array('fo.JOB_CHAIN_NODE_ID', 'fcn.ID'))
                .$sql->LeftJoin('JOC_JOB_CHAINS fc', array('fcn.JOB_CHAIN_ID', 'fc.ID'))
   ...
   public function getJobChainInfos($job_chain_id) {   
        ...
        $qry = $sql->Select(array('fjc.PATH','fs.NAME','fs.IP_ADDRESS','fs.TCP_PORT'))
                .$sql->From(array('JOC_JOB_CHAINS fjc'))
                .$sql->LeftJoin('JOC_SPOOLERS fs',array('fjc.spooler_id','fs.id'))
                .$sql->Where($Fields);
   ...
   public function getJobInfos($job_id,$status="UNKNOWN") {   
        ...
        $qry = $sql->Select(array('fj.PATH','fs.NAME','fs.IP_ADDRESS','fs.TCP_PORT'))
                .$sql->From(array('JOC_JOBS fj'))
                .$sql->LeftJoin('JOC_SPOOLERS fs',array('fj.spooler_id','fs.id'))
                .$sql->Where($Fields);
        ...
        if ($line = $data->sql->get_next($res)) {
            ...
            $hostname = $line['IP_ADDRESS'];
            $port = $line['TCP_PORT'];
            $res = $data->sql->query( 'update JOC_JOBS set STATE="'.$status.'",CRC="" where id='.$job_id);
   ...
   public function getJobChainNodeInfos($job_chain_node_id) {   
        ...
        $qry = $sql->Select(array('fjc.PATH','fjcn.STATE','fs.NAME','fs.IP_ADDRESS','fs.TCP_PORT'))
                .$sql->From(array('JOC_JOB_CHAIN_NODES fjcn'))
                .$sql->LeftJoin('JOC_JOB_CHAINS fjc',array('fjcn.job_chain_id','fjc.id'))
                .$sql->LeftJoin('JOC_SPOOLERS fs',array('fjcn.spooler_id','fs.id'))
                .$sql->Where($Fields);
