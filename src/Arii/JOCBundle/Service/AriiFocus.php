...
class AriiFocus
{
    ...
    public function cache($data, $mode='',$debug) {   
        ...
        if (isset($SpoolerID[$id])) {
            ...
            $qry = 'update JOC_SPOOLERS set TIME="'.$this->CorrectDatetime($time).'",'
                    . 'NAME="'.$spooler.'",'
            ...
            // $qry = 'insert into JOC_SPOOLERS (NAME,HOST,IP_ADDRESS,TCP_PORT,CONNECTION,SPOOLER_RUNNING_SINCE,STATE,VERSION,UPDATED,LOG_FILE,PID,NEED_DB,UDP_PORT,CONFIG_FILE,DB,TIME,WAITS,WAIT_UNTIL,LOOPS) values( "'.$spooler.'","'.$host.'","'.$ip_address.'","'.$tcp_port.'","","","","","","","","","","","","","","","")';
            $qry = 'insert into JOC_SPOOLERS (NAME,HOST,IP_ADDRESS,TCP_PORT) values( "'.$spooler.'","'.$host.'","'.$ip_address.'","'.$tcp_port.'")';
            $res = $data->sql->query( $qry );
            // on recuperer a nouveau l'id
            $qry = 'select ID from JOC_SPOOLERS where IP_ADDRESS="'.$ip_address.'" and TCP_PORT="'.$tcp_port.'"'; 
            $res = $data->sql->query( $qry );
...
*/        
        ...
        $qry = 'select ID,PATH,CRC from JOC_JOBS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_job=0;
        while ( $line = $data->sql->get_next($res) ) {
                $id  =  $line['ID'];
        ...
        $qry = 'select ID,TASK,CRC from JOC_TASKS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_task=0;
        while ( $line = $data->sql->get_next($res) ) {
                $id  =  $line['ID'];
        ...
        // image des verrous
        $qry = 'select ID,PATH,CRC from JOC_LOCKS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_lock=0;
        while ( $line = $data->sql->get_next($res) ) {
                $lock  =  $line['PATH'];
        ...
        $qry = 'select ID,NAME,LOCK_ID,JOB_ID,CRC from JOC_LOCKS_USE where spooler_id='.$spooler_id;
        $res = $data->sql->query( $qry );
        $c_lockuse=0;
        while ( $line = $data->sql->get_next($res) ) {
                $id  =  $line['ID'];
        ...
        // image des schedules
        $qry = 'select ID,PATH,CRC from JOC_SCHEDULES where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_schedule=0;
        while ( $line = $data->sql->get_next($res) ) {
                $lock  =  $line['PATH'];
        ...
        // image des periodes
        $qry = 'select ID,SCHEDULE_ID,DATE,MONTHS,DAYS,START_TIME,CRC from JOC_PERIODS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_period=0;
        while ( $line = $data->sql->get_next($res) ) {
                $id  =  $line['ID'];
        ...
        $qry = 'select ID,PATH,CRC from JOC_JOB_CHAINS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_jobchain=0;
        while ( $line = $data->sql->get_next($res) ) {
                $id  =  $line['ID'];
        ...
        $qry = 'select ID,JOB_CHAIN_ID,STATE,CRC from JOC_JOB_CHAIN_NODES where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_jobchainnode=0;
        while ( $line = $data->sql->get_next($res) ) {
                $id  =  $line['ID'];
        ...
        // et enfin les ordres
        $qry = 'select ID,PATH,CRC from JOC_ORDERS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_order=0;
        while ( $line = $data->sql->get_next($res) ) {
                $id  =  $line['ID'];
        ...
        $qry = 'select ID,ORDER_ID,NAME,CRC from JOC_PAYLOADS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_payload=0;
        while ( $line = $data->sql->get_next($res) ) {
        ...
        // image des parametres
        $qry = 'select ID,JOB_ID,NAME,CRC from JOC_JOB_PARAMS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_params=0;
        while ( $line = $data->sql->get_next($res) ) {
        ...
        $qry = 'select ID,DIRECTORY,REGEX,CRC from JOC_FILE_ORDERS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_file=0;
        while ( $line = $data->sql->get_next($res) ) {
                $id  =  $line['ID'];
        ...
        $qry = 'select ID,PATH,CRC from JOC_PROCESS_CLASSES where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_process=0;
        while ( $line = $data->sql->get_next($res) ) {
                $process  =  $line['PATH'];
        ...
        $qry = 'select ID,HOST_IP,PORT,CRC from JOC_CONNECTIONS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_connect=0;
        $ConnectionID=array();
        while ( $line = $data->sql->get_next($res) ) {
...
/* Ca sert ?
     if (isset($Result['spooler']['answer']['state']['order_id_spaces']['order_id_space'])) {
        $qry = 'select ID,NAME,JOB_CHAIN_ID,CRC from JOC_ORDER_ID_SPACES where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_space=0;
        $SpaceID=array();
        while ( $line = $data->sql->get_next($res) ) {
...
*/    
    if (isset($Result['spooler']['answer']['state']['remote_schedulers']['remote_scheduler'])) {
        // remote schedulers 
        $qry = 'select ID,IP,TCP_PORT,CRC from JOC_REMOTE_SCHEDULERS where spooler_id='.$spooler_id; 
        $res = $data->sql->query( $qry );
        $c_remote=0;
        while ( $line = $data->sql->get_next($res) ) {
                $id  =  $line['ID'];
                         ...
                         for($i=1;$i<=$params_count;$i++) {
                             ...
                             if (isset($ParamsID[$param])) {
                                     ...
                                     if ($ParamsCRC[$param]!=$crc) {
                                         $qry = 'update JOC_JOB_PARAMS set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$job_params;
                                         array_push($Queries, $qry);
                                     }
                             }
                             else {
                                     $qry = 'insert into JOC_JOB_PARAMS (SPOOLER_ID,JOB_ID,NAME,VALUE,UPDATED,CRC) values ( '.$spooler_id.','.$job_params.',"'.$name.'","'.$value.'",'.$timestamp.',"'.$crc.'" )';
                                     array_push($Queries, $qry);
                            }                                                            
                         }
                    }
                            ...
                            while (isset($Tasks[$nt])) {
                                    if (isset($Tasks[$nt]['attr'])) {
                                            ...
                                            if (isset($TaskID[$task])) {
                                                    $TaskDel[$task]=-1;
                                                    if ($TaskCRC[$task]!=$crc) {
                                                        $qry = 'update JOC_TASKS set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$TaskID[$task];
                                                        array_push($Queries, $qry);
                                                    }
                                            }
                                            else {
                                                    $qry = 'insert into JOC_TASKS (TASK,SPOOLER_ID,JOB_ID,STATE,NAME,RUNNING_SINCE,ENQUEUED,START_AT,CAUSE,STEPS,PID,PRIORITY,FORCE_START,UPDATED,CRC) values ( '.$task_id.', '.$spooler_id.', '.$job_id.', "'.$state.'", "'.$name.'", "'.$running_since.'", '.$enqueued.', "'.$start_at.'", "'.$cause.'", "'.$steps.'", "'.$pid.'", "'.$priority.'", "'.$force_start.'",'.$timestamp.',"'.$crc.'" )';
                                                    array_push($Queries, $qry);
                                            }
                                            //	print $qry;
                                            //$res = $data->sql->query( $qry );
                                    ...
                                    while (isset($Locks['lock.use'][$nl]['attr'])) {
                                            ...
                                            if (isset($LockUseID[$lockjob])) {
                                                    ...
                                                    if ($LockUseCRC[$lockjob]!=$crc) {
                                                        $qry = 'update JOC_LOCKS_USE set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$LockUseID[$lockjob];
                                                        array_push($Queries, $qry);
                                                    }
                                            }
                                            else {
                                                    $qry = 'insert into JOC_LOCKS_USE (SPOOLER_ID,NAME,LOCK_ID,JOB_ID,EXCLUSIVE,IS_AVAILABLE,IS_MISSING,UPDATED,CRC) values ('.$spooler_id.', "'.$lock.'", '.$lock_id.', '.$job_id.', '.$exclusive.','.$is_available.','.$is_missing.','.$timestamp.',"'.$crc.'" )';
                                                    array_push($Queries, $qry);
                                            }
                                            $nl++;
                                    }
                ...
                while (isset($Jobs[$n])) {
                        ...
                        if (isset($JobChainID[$job_chain])) {
                            ...
                            if ($JobChainCRC[$job_chain]!=$crc) {
                                $qry = 'update JOC_JOB_CHAINS set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$job_chain_id;
                                array_push($Queries, $qry);
                            }
                        }
                        else {
                                $job_chain_id= 'null';
                                $qry = 'insert into JOC_JOB_CHAINS (SPOOLER_ID,NAME,PATH,TITLE,ORDER_ID_SPACE,STATE,ORDERS_RECOVERABLE,ORDERS,RUNNING_ORDERS,MAX_ORDERS,LAST_WRITE_TIME,UPDATED,CRC) values ( '.$spooler_id.', "'.$name.'", "'.$path.'", "'.$title.'", '.$order_id_space.', "'.$state.'", '.$orders_recoverable.', '.$orders.', '.$running_orders.', '.$max_orders.', "'.$last_write_time.'",'.$timestamp.',"'.$crc.'" )';
                                array_push($Queries, $qry);
                        }
                        ...
                        if (isset(  $Jobs[$n]['file_order_source'] ) ) {
                            ...
                            while (isset($Files[$nf])) {
                                    ...
                                    if (isset($FileID[$file])) {
                                            ...
                                            $file_id = $FileID[$file];
                                            if ($FileCRC[$file]!=$crc) {
                                                $qry = 'update JOC_FILE_ORDERS set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'",ALERT_WHEN_DIRECTORY_MISSING='.$missing.' where id='.$file_id;
                                                array_push($Queries, $qry);
                                            }
                                    }
                                    else {
                                        // inutile si la chaine n'est pas encore dans la base de donnees
                                            $file_id = 'null';
                                        if ($job_chain_id>0) {
                                            $qry = 'insert into JOC_FILE_ORDERS (SPOOLER_ID,JOB_CHAIN_ID,DIRECTORY,REGEX,DELAY_AFTER_ERROR,RETRY,ALERT_WHEN_DIRECTORY_MISSING,UPDATED,CRC) values ( '.$spooler_id.', '.$job_chain_id.', "'.$directory.'", "'.$regex.'", '.$delay.', '.$repeat.', '.$missing.', '.$timestamp.',"'.$crc.'" )';
                                            array_push($Queries, $qry);
                                        }
                                    }
                                    $nf++;
    ...
    //                            print_r($Jobs[$n]['job_chain_node']);
                                ...
                                while (isset($Nodes[$nn])) {
                                        ...
                                        if (isset($NodeID[$node])) {
                                                ...
                                                $node_id = $NodeID[$node];
                                                if ($NodeCRC[$node]!=$crc) {
                                                    $qry = 'update JOC_JOB_CHAIN_NODES set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$NodeID[$node];
                                                    array_push($Queries, $qry);
                                                }
                                        }
                                        else {
                                            // inutile si la chaine n'est pas encore dans la base de donnees
                                                $node_id = 'null';
                                            if ($job_chain_id>0) {
                                                $qry = 'insert into JOC_JOB_CHAIN_NODES (SPOOLER_ID,JOB_CHAIN_ID,JOB_ID,CHAIN_ID,STATE,TITLE,NEXT_STATE,ERROR_STATE,ACTION,ORDERING,UPDATED,CRC) values ( '.$spooler_id.', '.$job_chain_id.', '.$job_id.', '.$chain_id.', "'.$state.'", "'.$title.'", '.$next_state.', '.$error_state.', '.$action.', '.$ordering.','.$timestamp.',"'.$crc.'" )';
                                                array_push($Queries, $qry);
                                            }
                                        }
                                        $ordering++;
                                                            ...
                                                            for($i=1;$i<=$params_count;$i++) {
                                                                ...
                                                                if (isset($PayloadID[$payload])) {
                                                                        ...
                                                                        if ($PayloadCRC[$payload]!=$crc) {
                                                                            $qry = 'update JOC_PAYLOADS set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$PayloadID[$payload];
                                                                            array_push($Queries, $qry);
                                                                        }
                                                               }
                                                                else {
                                                                        $qry = 'insert into JOC_PAYLOADS (SPOOLER_ID,ORDER_ID,NAME,VALUE,UPDATED,CRC) values ( '.$spooler_id.','.$order_payload.',"'.$name.'","'.$value.'",'.$timestamp.',"'.$crc.'" )';
                                                                        array_push($Queries, $qry);
                                                               }     
                                                            }
        ...
        if (isset($Result['spooler']['answer']['state']['locks']['lock'])) {
                ...
                while (isset($Locks[$n])) {
                        ...
                        if (isset($LockID[$lock])) {
                            $LockDel[$lock]=-1;
                            if ($LockCRC[$lock]!=$crc) {
                                $qry = 'update JOC_LOCKS set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$LockID[$lock];
                                array_push($Queries, $qry);
                            }
                        }
                        else {
                                $qry = 'insert into JOC_LOCKS (NAME,PATH,SPOOLER_ID,IS_FREE,STATE,UPDATED,CRC) values ( "'.$name.'", "'.$path.'",'.$spooler_id.','.$is_free.', "'.$state.'",'.$timestamp.',"'.$crc.'" )';		
                                array_push($Queries, $qry);
                        }
                        $n++;
                }
                ...
                while (isset($Schedules[$n])) {
                        ...
                        if (isset($ScheduleID[$schedule])) {
                            ...
                            if ($ScheduleCRC[$schedule]!=$crc) {
                                $qry = 'update JOC_SCHEDULES set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$ScheduleID[$schedule];
                                array_push($Queries, $qry);
                            }                        
                        }
                        else {
                                $qry = 'insert into JOC_SCHEDULES (NAME,PATH,TITLE,SPOOLER_ID,ACTIVE,STATE,VALID_TO,VALID_FROM,SUBSTITUTE_ID,UPDATED,CRC) values ( "'.$name.'", "'.$path.'", "'.$title.'",'.$spooler_id.','.$active.', "'.$state.'",'.$substitute_id.','.$valid_from.','.$valid_to.','.$timestamp.',"'.$crc.'" )';		
                                array_push($Queries, $qry);
                        }
                            ...
                            while (isset($Period[$np]['period']['attr'])) {   
                                ...
                                if (isset($PeriodID[$period])) {
                                    ...
                                    if ($PeriodCRC[$period]!=$crc) {
                                        $qry = 'update JOC_PERIODS set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$PeriodID[$period];
                                        array_push($Queries, $qry);      
                                    }
                                }
                                else { 
                                    $qry = 'insert into JOC_PERIODS (SPOOLER_ID,SCHEDULE_ID,DATE,MONTHS,DAYS,DAY_TYPE,BEGIN,END,START_TIME,START_TYPE,WHEN_HOLIDAY,UPDATED,CRC) values ( '.$spooler_id.','.$schedule_id.',"'.$date.'","'.$months.'","'.$days.'","'.$day_type.'",'.$begin.','.$end.',"'.$start_time.'",'.$start_type.','.$when_holiday.','.$timestamp.',"'.$crc.'" )';		
                                    array_push($Queries, $qry);
                                }
                                $np++;
                            }
            ...
            while (isset($Classes[$n])) {
                    ...
                    if (isset($ProcessClassID[$process])) {
                        ...
                        if ($ProcessClassCRC[$process]!=$crc) {
                            $qry = 'update JOC_PROCESS_CLASSES set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$ProcessClassID[$process];
                            array_push($Queries, $qry);
                            $nu++;
                        }
                    }
                    else {
                            $qry = 'insert into JOC_PROCESS_CLASSES (NAME,PATH,SPOOLER_ID,REMOTE_SCHEDULER,MAX_PROCESSES,PROCESSES,STATE,LAST_WRITE_TIME,UPDATED,CRC,ERROR) values ( "'.$name.'", "'.$path.'", '.$spooler_id.', "'.$remote_scheduler.'", "'.$max_processes.'", "'.$processes.'", "'.$state.'", "'.$last_write_time.'",'.$timestamp.',"'.$crc.'",'.$error.' )';		
                            array_push($Queries, $qry);
                            $ni++;
                    }
                    $n++;
            ...
            while (isset($Connections[$n])) {
                    ...
                    if (isset($ConnectionID[$connect])) {
                        ...
                        if ($ConnectionCRC[$connect]!=$crc) {                     
                            $qry = 'update JOC_CONNECTIONS set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$ConnectionID[$connect];
                            array_push($Queries, $qry);
                        }
                    }
                    else {
                            $qry = 'insert into JOC_CONNECTIONS (SPOOLER_ID,OPERATION_TYPE,RECEIVED_BYTES,RESPONSES,SEND_BYTES,STATE,HOST_IP,PORT,UPDATED,CRC) values ( '.$spooler_id.',"'.$operation_type.'", '.$received_bytes.', '.$responses.', '.$sent_bytes.', "'.$state.'", "'.$host_ip.'", '.$port.','.$timestamp.',"'.$crc.'" )';		
                            array_push($Queries, $qry);
                    }
                    $n++;
            }
            ...
            while (isset($Spaces[$n]['job_chain'][$nc]['attr']['job_chain'])) {
                ...
                if (isset($SpaceID[$idspace])) {
                    ...
                    if ($SpaceCRC[$idspace]!=$crc) {
                        $qry = 'update JOC_ORDER_ID_SPACES set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$SpaceID[$idspace];
                        array_push($Queries, $qry);
                    }
                }
                else {
                        $qry = 'insert into JOC_ORDER_ID_SPACES (SPOOLER_ID,NAME,JOB_CHAIN_ID,UPDATED,CRC) values ( '.$spooler_id.',"'.$name.'", '.$jc.','.$timestamp.',"'.$crc.'" )';		
                        array_push($Queries, $qry);
                }
                $nc++;
            }
            ...
            while (isset($Remotes[$n])) {
                    ...
                    if (isset($RemoteID[$remote])) {
                        if ($RemoteCRC[$remote]!=$crc) {
                            $RemoteDel[$remote]=-1;
                            $qry = 'update JOC_REMOTE_SCHEDULERS set '.$update.',UPDATED='.$timestamp.',CRC="'.$crc.'" where id='.$RemoteID[$remote];
                            array_push($Queries, $qry);
                        }
                    }
                    else {                                                                                                                                                                                                                                              
                            $qry = 'insert into JOC_REMOTE_SCHEDULERS (SPOOLER_ID,REMOTE_ID,IP,TCP_PORT,HOSTNAME,NAME,CONFIGURATION_CHANGED_AT,CONFIGURATION_TRANSFERED_AT,CONNECTED,CONNECTED_AT,DISCONNECTED_AT,VERSION,ERROR,ERROR_AT,UPDATED,CRC)'
...
if (isset($JobDel))  {  
    $qry = 'delete from JOC_JOB_RUNTIMES where job_id in ('.implode(',',$JobDel).')'; 
    $res = $data->sql->query( $qry );
    $qry = 'delete from JOC_JOB_STATUS where job_id in ('.implode(',',$JobDel).')'; 
    $res = $data->sql->query( $qry );
}
if (isset($OrderDel))  {  
    $qry = 'delete from JOC_ORDER_STATUS where order_id in ('.implode(',',$OrderDel).')'; 
    $res = $data->sql->query( $qry );
    $qry = 'delete from JOC_ORDER_STEP_STATUS where order_id in ('.implode(',',$OrderDel).')'; 
    $res = $data->sql->query( $qry );
    $qry = 'delete from JOC_ORDER_STEP_RUNTIMES where order_id in ('.implode(',',$OrderDel).')'; 
    $res = $data->sql->query( $qry );
    $qry = 'delete from JOC_ORDER_RUNTIMES where order_id in ('.implode(',',$OrderDel).')'; 
    $res = $data->sql->query( $qry );
    $qry = 'delete from JOC_PAYLOADS where order_id in ('.implode(',',$OrderDel).')'; 
    $res = $data->sql->query( $qry );
}
...
elseif ($mode=='debug') {
    ...
    private function QueryDel($table, $Tableau, $id = 'ID') {
        ...
        return 'delete from JOC_'.$table.' where '.$id.' in ('.implode(',',$Del).')';
    }
